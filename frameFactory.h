...
public:
  ...
  std::vector<Frame*> getTwoWayFrames(const std::string&);

  std::vector<Frame*> getRandFrames(const std::string&, const int);
...
private:
  ...
  //two-way multi-surfaces

  std::map<std::string, std::vector<SDL_Surface*> > twoWaySurfaces;
  ...
  // The next map stores Frames for Two-way multi-frame sprites;
  std::map<std::string, std::vector<Frame*> > twoWayFrames;
