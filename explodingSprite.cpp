...
ExplodingSprite::ExplodingSprite(const MultiSprite& s):
   Sprite(s.getName(),s.getPosition(), s.getVelocity(), s.getFrame()),
   chunks(),
   freeList(),
   frames()
{
...
void ExplodingSprite::reset(const Vector2f& pos) {
   if(chunks.empty()){
      ...
      while(it != chunks.end()){
         Sint16 sx = it->getFrame()->getSourceX();
         Sint16 sy = it->getFrame()->getSourceY();
         it->reset();
         Vector2f vec(pos[0] + sx, pos[1] + sy);
         it->setPosition(vec);
         ++it;
      }
   }
